...
type InitWith<T> = {
	...
	letAsync<R>(fn: (it: T) => Promise<R>): Promise<R extends void ? T : R>;
	alsoAsync(fn: (it: T) => Promise<void>): Promise<T>;
	produceAsync(fn: (it: T) => Promise<void>): Promise<T>;
	readonly taskMod: With<T>;
};
 ...
 * @alias initWith
 */
export const iw: typeof initWith = initWith;
